    ...
    function renderOptions() {
      ...
      modelSelect.onchange = () => {
        ...
        if (preparationStatus) {
          ...
          const testedRuntime = String(preparation.tested_runtime || preparation.actual_runtime || "unknown").trim().toUpperCase();
          if (state === "prepared" && preparedModel === val) {
            preparationStatus.textContent = `Prepared on ${testedRuntime}${preparation.finished_at ? ` · ${preparation.finished_at}` : ""}. Scoring validates the selected runtime at use time.`;
